...
func main() {
	...
	if err != nil {
		log.Fatalf("claude-db: cannot read settings.json: %v", err)
	}
	...
	if err != nil {
		log.Fatalf("claude-db: failed to fetch initial token for profile %q: %v", resolvedProfile, err)
	}
	...
	if inferenceUpstream == "" {
		...
		if err != nil {
			log.Fatalf("claude-db: failed to discover host for profile %q: %v\nRun 'databricks auth login --profile %s' first",
				resolvedProfile, err, resolvedProfile)
		}
		log.Printf("claude-db: discovered host: %s", host)
		databricksHost = host

		inferenceUpstream = ConstructGatewayURL(host, initialToken)
		log.Printf("claude-db: upstream: %s", inferenceUpstream)
	...
	if err != nil {
		log.Fatalf("claude-db: failed to start proxy: %v", err)
	}
	...
	if needsFullSetup {
		...
		log.Printf("claude-db: self-setup complete — profile=%s, host=%s, gateway=%s",
			resolvedProfile, databricksHost, inferenceUpstream)
	} else {
		if err := sm.SaveAndOverwrite(proxyURL); err != nil {
			log.Fatalf("claude-db: failed to patch settings.json: %v", err)
		}
	}
	defer func() {
		if err := sm.Restore(); err != nil {
			log.Printf("claude-db: warning: failed to restore settings.json: %v", err)
		}
	}()

	// --- Log startup info ---
	log.Printf("claude-db: proxy listening on %s, profile=%s, upstream=%s",
